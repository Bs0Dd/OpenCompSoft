...
local picpat = fs.path(sys.getCurrentScript()).."/Pictures/"

local gamelogo = image.load(picpat.."vvprom.pic")
local bcontrp = image.load(picpat.."bcontr.pic")
local editrp = image.load(picpat.."editor.pic")
local taskp = image.load(picpat.."task.pic")
local cidlogo = image.load(picpat.."cidlogo.pic")
local comlogo = image.load(picpat.."comlogo.pic")
local infoic = image.load(picpat.."info.pic")
local helpic = image.load(picpat.."help.pic")
local exitic = image.load(picpat.."exit.pic")
local contric = image.load(picpat.."controls.pic")
...
local function pull(arg1, arg2)
	if type(arg1) == "string" then
		while true do
			local pres = table.pack(event.pull())
			if pres[1] == arg1 then return table.unpack(pres) end
		end
	else
		local deadline = computer.uptime() + (arg1 or 0)
		repeat
			local pres = table.pack(event.pull(deadline - computer.uptime()))
			if not arg2 or pres[1] == arg2 then return table.unpack(pres) end
...
local function loadTsk()
	local rawf, why = fs.open(fs.path(sys.getCurrentScript())..'/Tasks/task'..taskNum..'.vtf', 'r')
	if not rawf then
		if taskNum == 1 then
			GUI.alert("Error: Can't open first task file: "..(why or "unknown reason"))
			work = false
			return
		else
			return false
	...
	tsk, why = srz.deserialize(rawf:readAll())
	if not tsk then
		GUI.alert("Error: Can't load task file: "..(why or "unknown reason"))
		work = false
		return
	end
	taskNum = taskNum+1
